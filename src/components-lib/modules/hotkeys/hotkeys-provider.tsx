...
import BoundHotkeysProxyProviderProvider from "@modules-lib/hotkeys/bound-hotkeys-proxy-provider";
...
export const HotkeysProvider = ({
  ...
  const removeBoundHotkey = useCallback((hotkey: Hotkey) => {
    setBoundHotkeys((prev) =>
      prev.filter((h) => !Utilities.deepEqual(h, hotkey))
    );
  }, []);
